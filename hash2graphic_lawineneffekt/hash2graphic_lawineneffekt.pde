...
for (int i = 0; i < 100; i++) {
transformation2 = transformation4(transformation2, colors.length);
...
color[] getColors(String hash) {
  int colorCount = hash.chars().sum() % 7 + 5;
  int colorRange = hash.chars().sum() % 98;
  int indexJump = hash.chars().sum() % 2 + 1;

  int length = min(colorCount, colorsGlobal.length);
  length = max(3, colorCount);

  int start = floor(float(colorsGlobal.length) * float(colorRange) / 99.0);

  color[] trimmedColors = new color[length];

  for (int i = 0; i < length; i++) {
    int copyIndex = (start + i * indexJump) % colorsGlobal.length;
    trimmedColors[i] = colorsGlobal[copyIndex];
  }
...
int[][] transformation4(int[][] state, int neighborhoods) {
  int[][] nextState = state;//new int[state.length][state[0].length];
   int j = floor(random(state[0].length));//frameCount% state[0].length;
   int i = floor(random(state.length));//floor(float(frameCount)/state[0].length)%state.length;
   //println(i,j);
      if (i > 0 && state[i-1][j] == (state[i][j] + 1) % neighborhoods) {
        nextState[i][j] = state[i-1][j];
      } else if (i < state.length-1 && state[i+1][j] == (state[i][j] + 1) % neighborhoods) {
